...
export default class AreaSelectModal extends Component {
  ...
  areaItemCheckBoxClicked = (areaLevel, e, selectId) => {
    ...
    switch (areaLevel) {
      ...
      case 2:
        ...
        let selectedProviceId = selectId.toString().substr(0, 2) + "0000";
        this.setState({
      ...
      case 3:
        ...
        this.selectDisctrictById(selectId, checked);
        break;
    }
  }
  ...
  selectCityById = (id, checked) => {
    if (!id) {
      return;
    }
    let checkedAreaMap = this.state.checkedAreaMap;
    let selectedProviceId = id.toString().substr(0, 2) + "0000";
    let spId = parseInt(selectedProviceId);
    let scId = parseInt(id);
    checkedAreaMap[spId]['children'][scId]['checked'] = checked;
  ...
  selectDisctrictById = (id, checked) => {
    if (!id) {
      return;
    }
    let checkedAreaMap = this.state.checkedAreaMap;
    let selectedProviceId = id.toString().substr(0, 2) + "0000";
    let selectedCityId = id.toString().substr(0, 4) + "00";
    let spId = parseInt(selectedProviceId);
    let scId = parseInt(selectedCityId);
    checkedAreaMap[spId]['children'][scId]['children'][id]['checked'] = checked;

    this.setState({
      checkedAreaMap,
      selectedDisctrictId: id
    })
  }
  ...
  checkedGroupOnchange = (areaLevel, checkedList) => {
    ...
    switch (areaLevel) {
      ...
      case 3:
        let spid = this.state.selectedProviceId;
        let scid = this.state.selectedCityId;
        if(!spid || !scid){
          return;
        }
        let districtList = checkedAreaMap[spid]['children'][scid]['children'];
  ...
  getCheckedCityList = () => {
    ...
    let result = checkedList.map(item => parseInt(item));
    return result;
  }
  ...
  getCheckedDistrictList = () => {
    ...
    let list = cityData.children;
    let checkedList = Object.keys(list).filter(item => cityData.children[item].checked);
    return checkedList.map(item => parseInt(item));
  }
      ...
      >
        {
          this.state.areaList ?
            <Row>
              <Col span={5} className='padding-left'>
              ...
              <Col span={5} className='padding-left'>
                <div className='padding flex border-bottom border-right '>
                  <Checkbox disabled={this.props.shouldNotSave} checked={this.state.allDistrictChecked} onChange={(e) => { this.arealAllCheckBoxClicked(3, e) }}><span className='font-bold'>全选</span></Checkbox>
                </div>
                <div className='border-right' >
                  <div style={{ height: 600, overflowY: "auto" }}>
                    <Checkbox.Group disabled={this.props.shouldNotSave} value={checkedDistrictList} style={{ width: '100%' }} onChange={(e) => { this.checkedGroupOnchange(3, e) }}>
                      {
