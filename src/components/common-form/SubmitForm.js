    ...
    if (formItemList && formItemList.length > 0) {
      formItemList.forEach((item, i) => {
        ...
        switch (item.type) {
          ...
          case "AUTOCOMPLETE":
            const AUTOCOMPLETE = <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 12 }} label={label} key={field} style={style} colon={colon}>
              {
                getFieldDecorator(field, {
                  rules: rules
                })(
          ...
          // 输入框
          case "INPUT_NUMBER":
            const INPUT_NUMBER = <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 12 }} label={label} key={field} colon={colon} >
              {
                getFieldDecorator(field, {
                  initialValue: initialValue || "",
                  rules: rules
